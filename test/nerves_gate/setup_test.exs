defmodule NervesGate.SetupTest do
  ...
  setup do
    ...
    ops = %{
      network: fn config ->
        send(owner, {:network, config})
        {:ok, TestScenario.checks()}
      end,
      tailscale: fn token ->
        send(owner, {:tailscale, token})
        :ok
      end,
      start_tailscale: fn -> :ok end,
      poll_tailscale: fn -> :ok end,
      tail_status: fn -> %{online: true, ipv4: "100.64.0.10"} end,
      distribution: fn ip ->
        send(owner, {:distribution, ip})
        :ok
      end,
      cluster: fn ->
        send(owner, :cluster)
  ...
  test "the complete setup is three explicit, independently testable calls", %{
    ...
    assert_receive {:network, %Config{interface: "eth0", method: :dhcp}}
    ...
    assert_receive {:tailscale, ^token}
  ...
  test "invalid Internet settings fail before touching the network", %{setup: setup} do
    ...
    refute_receive {:network, _config}
  end
end
